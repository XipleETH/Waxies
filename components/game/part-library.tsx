...
export function PartLibrary({open,onOpenChange,onInspect,available}:{open:boolean;onOpenChange:(value:boolean)=>void;onInspect:(id:PartId)=>void;available:PartId[]}){
 const [query,setQuery]=useState(''),[cl,setCl]=useState('all'),[slot,setSlot]=useState('all'),[tab,setTab]=useState('cards'),[limit,setLimit]=useState(24);
 ...
 return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="catalog-dialog"><DialogTitle>El arsenal de Lunacia</DialogTitle><DialogDescription>192 partes estándar · 132 cartas Classic de nivel 1 · 87 variantes cosméticas documentadas</DialogDescription>
 ...
 <div className="catalog-filters"><div className="catalog-search"><Search size={16}/><Input aria-label="Buscar parte o habilidad" placeholder="Parte, carta o efecto…" value={query} onChange={e=>{setQuery(e.target.value);setLimit(24);}}/></div>
