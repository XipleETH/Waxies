...
import {
  ...
  parseMetadataFile,
  validLoadout,
  type AxieLoadout as Loadout,
} from '@/lib/game/axie';
import { RoninAxies } from './ronin-axies';
import { PARTS } from '@/lib/game/catalog';
export function AxieLoadout({
  axie,
  onLoad,
  onLab,
  onBrowse,
  ...
  try {
    metadataUrl =
      'https://metadata.axieinfinity.com/axie/' + parseAxieId(input);
  } catch {}
  async function importFile(file: File | undefined) {
    ...
    try {
      if (file.size > 100000)
        throw new Error('El archivo debe pesar menos de 100 KB.');
      onLoad(parseMetadataFile(JSON.parse(await file.text())));
    } catch (e) {
  ...
  async function submit() {
    ...
    try {
      const response = await fetch('/api/axie/' + id, {
      ...
      if (!response.ok || !validLoadout(data.axie))
        throw new Error(data.error ?? 'No se pudo cargar el Axie.');
      if (!controller.signal.aborted) onLoad(data.axie);
    } catch (e) {
  ...
  return (
    <section
      className={
        view ? 'loadout-card focused-loadout view-' + view : 'loadout-card'
      }
    >
