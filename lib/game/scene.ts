...
export function createEngine(host: HTMLElement, onState: (s: GameState) => void, onLoad: (error?: string) => void, onAvatar: (status:AvatarStatus)=>void=()=>{}, onLevel:(level:Dungeon)=>void=()=>{}): Engine {
  ...
  const reportHaptics=createGameplayHaptics();
  let pollPad=()=>{};
  ...
  const loop=(now:number)=>{
    ...
    reportHaptics(state,!editing&&!playback&&!replay);
    if(!editing&&!playback&&!replay)pollPad();
  ...
  pollPad=createGamepadControls({jump,pause,restart});
  const visibility=()=>{if(document.hidden)stopHaptics();if(document.hidden&&state.phase==='playing'){if(playback){playback.paused=true;onState({...state});return;}state.phase='paused';onState({...state});}};
