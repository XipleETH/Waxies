...
export interface TrapState { x:number;y:number;facing:1|-1;stage:AttackStage;timer:number;shield:boolean;energy:number;powered:boolean;shots:number;shotTimer:number;
 hp:number;shieldHp:number;maxShield:number;cycleShield:number;statuses:StatusMap;round:number;broken:boolean;breakUsed:boolean;struckUsed:boolean;draw:number;lastStand:number;aimY:number;hitThisAttack:boolean }
...
function fire(s:GameState,level:Dungeon,i:number){
 ...
 for(const vy of speeds){const aim=pattern==='sniper'?Math.atan2(t.aimY-t.y,Math.abs(s.x-t.x)):0;
...
export function stepHazards(s:GameState,level:Dungeon,dt:number,oldY:number,descending:boolean){
 tickCombat(s,dt);if(s.phase!=='playing')return;
 const h=s.hazards;
 // Aroma changes attack priority only within an assisted stomp's short reach.
 if(descending){const target=h.traps.find(t=>t.stage!=='disabled'&&t.statuses.aroma&&!t.statuses.stench&&Math.abs(s.x-t.x)<1.05&&oldY-.38>=t.y+.25&&s.y-.38<=t.y+.35&&lineClear(level,s.x,s.y,t.x,t.y+.74));if(target)s.x=target.x;}
 for(let i=0;i<level.traps.length;i++){
  ...
  if(descending&&Math.abs(s.x-t.x)<.75&&oldY-.38>=t.y+.25&&s.y-.38<=t.y+.35)stomp(s,level,i);
  ...
  if(t.stage==='idle'&&t.timer<=0){
   ...
   if(!skip&&targetOK&&(auto||(Math.abs(s.x-t.x)<(distant?9:4.2)&&Math.abs(s.y-t.y)<1.8&&lineClear(level,t.x,t.y,s.x,s.y)))){t.stage='warning';t.timer=ATTACK.warning;t.facing=s.x<t.x?-1:1;t.aimY=s.y;}
 ...
 for(const p of h.projectiles){
  ...
  if(blocked(level,p.x,p.y,ATTACK.projectileRadius)){
   ...
   if(['grass-snake','yam','garish-worm'].includes(p.part)&&p.vy<0&&level.traps[p.owner]&&rulesFor(s,level,p.owner,'hit').some(r=>r.status==='poison')){
    ...
    if(surface!==null&&p.x>1.25&&p.x<22.75)h.pools.push({id:h.nextId++,x:p.x,y:surface+.06,life:2.2,owner:p.owner,part:p.part});
   }continue;
  }
